...
class DeblurImageResultView extends StatelessWidget {
  const DeblurImageResultView({
    super.key,
  ...
  Widget build(BuildContext context) {
    return BlocListener<ShowResultViewModel, ShowResultViewDataHolder>(
      listener: (context, state) {
        switch (state.debluredImageResultState) {
          case null:
            break;
          case DebluredImageOnLoading():
            AppLoaderOverlayManager.showOverlay(
              widget: AppLottiePlayer(path: AppAssetManager.loadingLottie),
            );
            break;
          case DebluredImageOnError(error: _):
            ...
            break;
          case DebluredImageOnLoaded(result: final result):
            AppLoaderOverlayManager.hideOverlay();
